...
This file comes from https://github.com/robince/gcmi/blob/master/python/gcmi.py 
"""
...
def cmi_ggg(x, y, z, biascorrect=True, demeaned=False):
    ...
    if y.shape[1] != Ntrl or z.shape[1] != Ntrl:
        raise ValueError("number of trials do not match.")
        print("y shape[1]: ", y.shape[1], "z shape[1]:", z.shape[1])
...
def gccmi_ccd(x,y,z,Zm):
    ...
    if y.shape[1] != Ntrl or z.size != Ntrl:
        raise ValueError("number of trials do not match.")
        print("y shape[1]: ", y.shape[1], "z shape[1]:", z.shape)
